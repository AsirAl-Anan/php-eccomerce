...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-4 flex justify-center">
        <button id="toggleMonth" class="bg-blue-500 text-white px-4 py-2 rounded mx-2" onclick="showChart('month')">Sales by Month</button>
        <button id="toggleYear" class="bg-green-500 text-white px-4 py-2 rounded mx-2" onclick="showChart('year')">Sales by Year</button>
        <button id="toggleWeek" class="bg-purple-500 text-white px-4 py-2 rounded mx-2" onclick="showChart('week')">Sales by Week</button>
    </div>

    <!-- Canvas Elements for Charts -->
    <div id="chartContainer">
        <canvas id="monthlySalesChart" class="hidden" width="400" height="200"></canvas>
        <canvas id="yearlySalesChart" class="hidden" width="400" height="200"></canvas>
        <canvas id="weeklySalesChart" class="hidden" width="400" height="200"></canvas>
    </div>
</div>
...
<script>
   ...
   // Get sales data from PHP
  // Monthly sales chart initialization as shown previously
  const ctxMonthly = document.getElementById('monthlySalesChart').getContext('2d');
    const monthlySalesChart = new Chart(ctxMonthly, {
        type: 'bar',
        data: {
            labels: salesDays, // Data for monthly sales
            datasets: [{
                label: 'Total Sales',
                data: salesTotalsMonthly,
    ...
    // Weekly sales chart initialization as shown previously
    const ctxWeekly = document.getElementById('weeklySalesChart').getContext('2d');
    const weeklySalesChart = new Chart(ctxWeekly, {
        type: 'bar',
        data: {
            labels: salesDaysOfWeek, // Data for weekly sales
            datasets: [{
                label: 'Total Sales',
                data: salesTotalsWeekly,
    ...
    // Initial chart to show when the page loads
    showChart('month');
    function showChart(chart) {
        // Hide all charts
        document.getElementById('monthlySalesChart').classList.add('hidden');
        document.getElementById('yearlySalesChart').classList.add('hidden');
        document.getElementById('weeklySalesChart').classList.add('hidden');

        // Show the selected chart
        if (chart === 'month') {
            document.getElementById('monthlySalesChart').classList.remove('hidden');
            monthlySalesChart.update();  // Update the chart to ensure it's rendered correctly
        } else if (chart === 'year') {
            document.getElementById('yearlySalesChart').classList.remove('hidden');
            yearlySalesChart.update();  // Update the chart to ensure it's rendered correctly
        } else if (chart === 'week') {
            document.getElementById('weeklySalesChart').classList.remove('hidden');
            weeklySalesChart.update();  // Update the chart to ensure it's rendered correctly
        }
    }

    // Show the monthly chart by default
    showChart('month');
