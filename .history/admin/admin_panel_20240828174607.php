...
function updateChart(labels, salesData) {
    ...
    if (salesChart) {
        salesChart.destroy();  // Destroy existing chart to update with new data
    }
        ...
        options: {
            ...
            scales: {
                x: {
                    title: {
                        display: true,
                        text: labels.length > 7 ? 'Day of the Month' : 'Day of the Week'
                    },
                    ticks: {
                        autoSkip: false,
                        maxRotation: 45,
...
// Fetch monthly sales data by default
