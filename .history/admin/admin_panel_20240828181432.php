...
function fetchSalesData(timeframe) {
    fetch(`fetch_sales_data.php?timeframe=${timeframe}`)
        ...
        .then(data => {
            ...
            let yAxisMax = 20000;  // Default max for month and week
            let stepSize = 5000;

            if (timeframe === 'year') {
                yAxisMax = 240000;  // Set max to 240,000 for the yearly graph
                stepSize = 5000;    // Set step size to 5,000
            }
            ...
            salesChart = new Chart(ctx, {
                ...
                options: {
                    scales: {
                        y: {
                            beginAtZero: true,
                            max: yAxisMax,  // Use the dynamic max value
                            ticks: {
                                stepSize: stepSize, // Use the dynamic step size
                            }
                        }
                    }
                }
