                    ...
                    <div class="relative h-64">
                        ...
                        foreach ($images as $index => $image):
                            ...
                            <img src="<?php echo $imagePath; ?>" alt="<?php echo htmlspecialchars($product['name']); ?>"
                                class="absolute inset-0 w-full h-full object-cover <?php echo $index === 0 ? '' : 'hidden'; ?>">
                        <?php endforeach; ?>
                        <?php if (count($images) > 1): ?>
                            <button class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 p-2 rounded-r"
                                onclick="changeImage(this, -1)">❮</button>
                            <button class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 p-2 rounded-l"
                                onclick="changeImage(this, 1)">❯</button>
                        <?php endif; ?>
                    </div>
                    <div class="p-4">
                        ...
                        <div class="flex items-center mb-4">
                            <?php for ($i = 1; $i <= 5; $i++): ?>
                                <svg class="w-5 h-5 fill-current <?php echo $i <= $product['rating'] ? 'text-yellow-500' : 'text-gray-400'; ?>"
                                    viewBox="0 0 24 24">
                                    <path
