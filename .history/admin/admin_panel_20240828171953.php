...
function getSalesDataByDay($conn) {
    $sql = "SELECT DAY(order_date) AS day, SUM(total_amount) AS total_sales
            FROM orders
            WHERE MONTH(order_date) = MONTH(CURDATE()) AND YEAR(order_date) = YEAR(CURDATE())
            GROUP BY DAY(order_date)
            ORDER BY DAY(order_date)";
    $result = $conn->query($sql);
    $days = array_fill(1, 31, 0); // Initialize an array for 31 days with 0 sales
    $sales = array_fill(1, 31, 0); // Initialize an array for 31 days with 0 sales

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $day = (int)$row['day'];
            $days[$day] = $day;
            $sales[$day] = $row['total_sales'];
        }
    }

    return ['days' => $days, 'sales' => $sales];
}

$salesDataByDay = getSalesDataByDay($conn);
...
<script>
   ...
   // Get sales data from PHP
   const salesDays = <?= json_encode(array_values($salesDataByDay['days'])) ?>;
    const salesTotals = <?= json_encode(array_values($salesDataByDay['sales'])) ?>;

    // Initialize the chart
     // Get sales data from PHP
     const salesDays = <?= $salesDays ?>; // All days of the month
    const salesTotals = <?= $salesTotals ?>; // Sales data including zeros
    ...
    const salesChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: salesDays.map(day => `Day ${day}`), // Label each day
            datasets: [{
                label: 'Total Sales',
                data: salesTotals,
            ...
            scales: {
                x: {
                    ...
                    ticks: {
                        ...
                        maxRotation: 45, // Rotate labels to avoid overlap if necessary
                        minRotation: 45
                    }
                },
                y: {
