...
<body>
    ...
    <div class="bg-gray-100 p-4 rounded mb-4" id="comment-<?php echo $comment['id']; ?>">
        ...
        <?php if (isLoggedIn() && $_SESSION['id'] == $comment['user_id']): ?>
            <div class="mt-2">
                <button onclick="editComment(<?php echo $comment['id']; ?>)" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded mr-2">Edit</button>
                <button onclick="deleteComment(<?php echo $comment['id']; ?>)" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded">Delete</button>
            </div>
        <?php endif; ?>
    </div>
<?php endforeach; ?>
