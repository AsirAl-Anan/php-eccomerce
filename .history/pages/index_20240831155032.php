    ...
    <?php foreach ($carousel_slides as $index => $slide): ?>
        <div id="slide<?php echo $index + 1; ?>" class="carousel-item relative w-full">
            ...
            <div class="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 bg-opacity-50 text-white p-5 rounded text-center">
                ...
                transition-all duration-500 ease-in-out shadow-md hover:scale-105 hover:shadow-lg before:absolute before:top-0 before:-left-full before:w-full before:h-full before:bg-gradient-to-r before:from-[#009b49] before:to-[rgb(105,184,141)] before:transition-all before:duration-500 before:ease-in-out before:z-[-1] before:rounded-xl hover:before:left-0 text-[#fff]""><?php echo htmlspecialchars($slide['button_text']); ?></a>
            </div>
