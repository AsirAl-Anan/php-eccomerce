...
// Fetch offer end time
$sql = "SELECT offer_end_time FROM offer_settings WHERE id = 1";
$result = $conn->query($sql);
$offer_end_time = null;
if ($result->num_rows > 0) {
...
<div class=""  id="featured">
  ...
  <?php displayProducts($products); ?> 
    ...
    <div class="carousel-container">
        <div class="carousel">
            <?php foreach ($carousel_slides as $index => $slide): ?>
                <div id="slide<?php echo $index + 1; ?>" class="absolute w-full h-full transition-opacity duration-500 <?php echo $index === 0 ? 'opacity-100' : 'opacity-0'; ?>">
                    <img src="<?php echo htmlspecialchars($slide['image_file']); ?>" class="w-full h-full object-cover" alt="<?php echo htmlspecialchars($slide['title']); ?>" />
                    <div class="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 bg-opacity-50 text-white p-5 rounded text-center">
                        <h2 class="text-lg sm:text-2xl md:text-3xl font-bold"><?php echo htmlspecialchars($slide['title']); ?></h2>
                        <p class="mt-2 text-sm sm:text-base md:text-lg"><?php echo htmlspecialchars($slide['description']); ?></p>
                        <a href="<?php echo htmlspecialchars($slide['button_link']); ?>" class="mt-4 inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"><?php echo htmlspecialchars($slide['button_text']); ?></a>
            ...
            <?php endforeach; ?>
            <button onclick="prevSlide()" class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 rounded-full p-2">❮</button>
            <button onclick="nextSlide()" class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 rounded-full p-2">❯</button>
        </div>
        <div class="timer">
            <h3 class="text-xl font-bold mb-4">Offer Ends In:</h3>
            <div id="countdown" class="flex flex-col items-center">
                <p class="text-2xl font-bold"><span id="days">00</span>d</p>
                <p class="text-2xl font-bold"><span id="hours">00</span>h</p>
                <p class="text-2xl font-bold"><span id="minutes">00</span>m</p>
                <p class="text-2xl font-bold"><span id="seconds">00</span>s</p>
            </div>
        </div>
    </div>
...
<!-- drawer -->
    ...
    <script>
      ...
      let currentSlide = 0;
        ...
        // Countdown Timer
        const offerEndTime = new Date("<?php echo $offer_end_time; ?>").getTime();
