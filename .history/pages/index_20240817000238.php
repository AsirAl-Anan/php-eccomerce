    ...
    fetch('../admin/cart.php?action=get_cart_data')
        ...
        .then(data => {
            ...
            if (data.cartItems.length === 0) {
                ...
                let html = '<table class="w-full"><thead><tr><th>Product</th><th>Quantity</th><th>Price</th><th>Total</th></tr></thead><tbody>';
                data.cartItems.forEach(item => {
                    html += `<tr>
                        <td>
                            <div class="flex items-center">
                                <img src="../uploads/${item.image_file || 'default.jpg'}" alt="${item.name}" class="w-16 h-16 object-cover mr-2">
                                <span>${item.name}</span>
                            </div>
                        </td>
                        <td>${item.quantity}</td>
                ...
                html += '</tbody></table>';
                html += `<p class="mt-4 text-right font-bold">Total: $${parseFloat(data.total).toFixed(2)}</p>`;
                html += '<button onclick="checkout()" class="mt-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded w-full">Checkout</button>';
