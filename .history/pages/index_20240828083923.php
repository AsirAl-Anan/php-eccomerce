...
<div class=""  id="featured">
  ...
  <?php displayProducts($products); ?> 
    ...
    <div class="relative w-[85%] h-full overflow-hidden">
        <div id="carousel" class="flex transition-transform duration-500 ease-in-out h-full">
            <?php foreach ($offer_slides as $index => $slide): ?>
                <div class="w-full h-full flex-shrink-0">
                    <img src="<?php echo htmlspecialchars($slide['image_file']); ?>" class="w-full h-full object-cover" alt="offer image" />
                    <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
                        <div class="text-center text-white p-5 rounded">
                            <h2 class="text-lg sm:text-2xl md:text-3xl font-bold"><?php echo htmlspecialchars($slide['title']); ?></h2>
                            <p class="mt-2 text-sm sm:text-base md:text-lg"><?php echo htmlspecialchars($slide['description']); ?></p>
                            <a href="<?php echo htmlspecialchars($slide['button_link']); ?>" class="mt-4 inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition duration-300"><?php echo htmlspecialchars($slide['button_text']); ?></a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        <button onclick="moveSlide(-1)" class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 hover:bg-opacity-75 text-white font-bold py-2 px-4 rounded-full transition duration-300 z-10">❮</button>
        <button onclick="moveSlide(1)" class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 hover:bg-opacity-75 text-white font-bold py-2 px-4 rounded-full transition duration-300 z-10">❯</button>
    </div>
