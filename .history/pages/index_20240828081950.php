...
<div class=""  id="featured">
  ...
  <?php displayProducts($products); ?> 
    ...
    <div class="carousel w-[85%] h-full relative overflow-hidden">
        <?php foreach ($offer_slides as $index => $slide): ?>
            <div id="slide<?php echo $index + 1; ?>" class="carousel-item absolute w-full h-full transition-opacity duration-300 ease-in-out <?php echo $index === 0 ? 'opacity-100' : 'opacity-0'; ?>">
        ...
        <?php endforeach; ?>
        <div class="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
            <button onclick="moveSlide(-1)" class="bg-black bg-opacity-50 hover:bg-opacity-75 text-white font-bold py-2 px-4 rounded-full transition duration-300">❮</button>
            <button onclick="moveSlide(1)" class="bg-black bg-opacity-50 hover:bg-opacity-75 text-white font-bold py-2 px-4 rounded-full transition duration-300">❯</button>
        </div>
    </div>
