...
$query = "SELECT * FROM offer_carousel WHERE is_active = 1 AND start_date <= '$current_time' AND end_date >= '$current_time' ORDER BY start_date ASC";
$result = mysqli_query($connection, $query);
$offers = mysqli_fetch_all($result, MYSQLI_ASSOC);

// Get the end time of the first active offer
$end_time = !empty($offers) ? strtotime($offers[0]['end_date']) : 0;
?>
...
<div class=""  id="featured">
  ...
  <?php displayProducts($products); ?> 
    ...
    <!-- Offer Carousel (85% width) -->
    <div class="w-[85%] h-full">
        <div class="carousel w-full h-full">
            <?php foreach ($offers as $index => $offer): ?>
                <div id="offer<?php echo $index + 1; ?>" class="carousel-item relative w-full">
                    <img src="../uploads/offers/<?php echo htmlspecialchars($offer['image_file']); ?>" class="w-full h-full object-cover" alt="offer image unavailable" />
                    <div class="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 bg-opacity-50 text-white p-4 rounded text-center">
                        <h2 class="text-lg sm:text-xl md:text-2xl font-bold"><?php echo htmlspecialchars($offer['title']); ?></h2>
                        <p class="mt-2 text-sm sm:text-base"><?php echo htmlspecialchars($offer['description']); ?></p>
                        <a href="<?php echo htmlspecialchars($offer['button_link']); ?>" class="mt-3 inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded text-sm"><?php echo htmlspecialchars($offer['button_text']); ?></a>
                    </div>
                    <div class="absolute left-2 right-2 top-1/2 flex -translate-y-1/2 transform justify-between">
                        <a href="#offer<?php echo ($index === 0) ? count($offers) : $index; ?>" class="btn btn-circle btn-sm">❮</a>
                        <a href="#offer<?php echo ($index === count($offers) - 1) ? 1 : $index + 2; ?>" class="btn btn-circle btn-sm">❯</a>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    <!-- Timer (15% width) -->
    <div class="w-[15%] h-full bg-gray-800 text-white flex flex-col justify-center items-center">
        <h3 class="text-lg font-bold mb-2">Offer Ends In:</h3>
