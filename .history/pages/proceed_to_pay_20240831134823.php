...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            $first_product_id = $order_details['items'][0]['product_id'];
$stmt = $conn->prepare("INSERT INTO orders (user_id, product_id, total_amount, status, shipping_address) VALUES (?, ?, ?, 'Processing', ?)");
$stmt->bind_param("iids", $order_details['user_id'], $first_product_id, $order_details['total_amount'], $order_details['shipping_address']);
$stmt->execute();
