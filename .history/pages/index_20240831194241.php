    ...
    <?php foreach ($carousel_slides as $index => $slide): ?>
        <div id="slide<?php echo $index + 1; ?>" class="carousel-item relative w-full">
            ...
            <div class="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 bg-opacity-50 text-white p-5 rounded text-center">
                ...
                <a href="<?php echo htmlspecialchars($slide['button_link']); ?>" class="mt-4 inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded
                "><?php echo htmlspecialchars($slide['button_text']); ?></a>
            </div>
...
<div class=""  id="featured">
  ...
  <form method="GET" action="" class="mb-4 flex items-center justify-center space-x-3">
    <label for="sort" class="text-sm font-medium text-gray-700">Sort by:</label>
    <select 
        id="sort" 
        name="sort" 
        onchange="this.form.submit()" 
        class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-48 p-2.5 shadow-sm">
   ...
   $sort = $_GET['sort'] ?? '';
$products = getProductsForPage($conn, $currentPage, $sort);
   ?>
  ...
  <div class="container mx-auto px-4">
    ...
    <form id="subscribeForm" class="flex flex-col md:flex-row justify-center items-center" method="POST">
      <input
        type="email"
        name="email"
        placeholder="Enter your email"
        class="p-3 border rounded w-full text-white md:w-2/3 lg:w-1/2 mb-4 md:mb-0 md:mr-4"
        required
      />
      <button
        type="submit"
    ...
    <p id="subscriptionMessage" class="hidden text-green-500 text-center mt-4">Subscribed!</p>
  </div>
</section>
...
<!-- drawer -->
    ...
    .then(result => {
        if (result.trim() === 'success') {
            document.getElementById("subscriptionMessage").textContent = "Subscribed!";
            document.getElementById("subscriptionMessage").classList.remove("hidden");
        } else if (result.trim() === 'error') {
            document.getElementById("subscriptionMessage").textContent = "You are already subscribed!";
            document.getElementById("subscriptionMessage").classList.remove("hidden");
        } else if (result.trim() === 'invalid') {
            document.getElementById("subscriptionMessage").textContent = "Please enter a valid email address!";
            document.getElementById("subscriptionMessage").classList.remove("hidden");
        }
    })
