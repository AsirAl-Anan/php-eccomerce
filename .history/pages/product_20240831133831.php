...
$canRate = false;
if (isLoggedIn()) {
  $sql = "SELECT * FROM orders WHERE user_id = ? AND product_id = ? AND status = 'Delivered'";
  $stmt = $conn->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_rating'])) {
    if (!isLoggedIn()) {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iiidds", $userId, $product_id, $rating, $review, $rating, $review);
...
<?php if ($canRate): ?>
    <div class="mt-8 bg-white p-6 rounded-lg shadow-md">
        <h3 class="text-2xl font-semibold mb-4">Rate this product</h3>
        <form action="" method="POST">
            <div class="mb-4">
                ...
                <select name="rating" id="rating" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    ...
                    <option value="5" <?php echo $userRating && $userRating['rating'] == 5 ? 'selected' : ''; ?>>5 Stars</option>
                    <option value="4" <?php echo $userRating && $userRating['rating'] == 4 ? 'selected' : ''; ?>>4 Stars</option>
                    <option value="3" <?php echo $userRating && $userRating['rating'] == 3 ? 'selected' : ''; ?>>3 Stars</option>
            ...
            <div class="mb-4">
                ...
                <textarea name="review" id="review" rows="4" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"><?php echo $userRating ? htmlspecialchars($userRating['review']) : ''; ?></textarea>
            </div>
